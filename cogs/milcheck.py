...
class Military(commands.Cog):
            ...
            for nation in nations:
                try:
                    ...
                    with requests.Session() as s:
                        ...
                        for x in txids['data']:
                            if x['note'] == 'Resupplying warchest' and start_time <= datetime.strptime(x['tx_datetime'], '%Y-%m-%dT%H:%M:%S%z') <= end_time:
                                success = True
                        if success:
    ...
    async def raids(self, ctx, *, arg=None):
        ...
        if webpage:
            ...
            await message.edit(content=f"Go to http://130.162.174.7/raids/{endpoint}", attachments=[])
            return
    ...
    async def damage(self, ctx, nation1=None, nation2=None):
        ...
        await message.edit(content=f"Go to http://130.162.174.7/damage/{endpoint}", attachments=[])
