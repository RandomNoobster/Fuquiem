...
class Military(commands.Cog):
    ...
    async def spy_msg(self, ctx):
        ...
        sheets = Sheets.from_files(pathlib.Path.cwd(
        ) / 'client_secrets.json', pathlib.Path.cwd() / 'storage.json')
        s = sheets.get(
            "https://docs.google.com/spreadsheets/d/11Q8H3VYeSHWHVuemFOz14DIS_PvMIAxMBloT-I8Zgt4/edit#gid=2086236902")
        ...
        con = (await utils.call(f"{{nations(alliance_id:8819 alliance_position:[2,3,4,5]) {{data{{id spy_attacks}}}}}}", convent_key))['data']['nations']['data']
        ...
        for k, v in to_send.items():
            ...
            res = requests.post('https://politicsandwar.com/api/send-message/', data={
                                'key': api_key, 'to': k, 'subject': "Spy targets", 'message': ingame_msg})
    ...
    async def wars(self):
        ...
        async def cthread(war, non_atom, atom):
            ...
            embed = discord.Embed(title=f"New {war_type} War", url=url,
                                  description=f"[{war['attacker']['nation_name']}](https://politicsandwar.com/nation/id={war['attacker']['id']}) declared a{'n'[:(len(war['war_type'])-5)^1]} {war['war_type'].lower()} war on [{war['defender']['nation_name']}](https://politicsandwar.com/nation/id={war['defender']['id']}) for the reason of: ```{war['reason']}```\n{footer}", color=0x2F3136)
        ...
        async def smsg(attacker_id, attack, war, atom, non_atom, peace):
            ...
            if war['att_alliance_id'] in ["4729", "8819"]:
                ...
                war_type = "Defensive"
            embed = discord.Embed(title=f"New {war_type} War", url=url,
                                  description=f"[{war['attacker']['nation_name']}](https://politicsandwar.com/nation/id={war['attacker']['id']}) declared a{'n'[:(len(war['war_type'])-5)^1]} {war['war_type'].lower()} war on [{war['defender']['nation_name']}](https://politicsandwar.com/nation/id={war['defender']['id']}) for the reason of: ```{war['reason']}```", color=0x2F3136)
            ...
            if found:
                ...
                print("could not find or create thread",
                      war['id'], peace, non_atom, atom)
                    ...
                    for war in all_wars:
                        if war['turnsleft'] <= 0:
                            declaration = datetime.strptime(
                                war['date'], '%Y-%m-%dT%H:%M:%S%z').replace(tzinfo=None)
                        ...
                        try:
                            ...
                            if len(done_war['attacks']) == 0:
                                attack = {"type": "EXPIRATION", "id": -1, "date": datetime.strftime(
                                    datetime.utcnow().replace(tzinfo=timezone.utc), '%Y-%m-%dT%H:%M:%S%z')}
    ...
    async def status(self, ctx, arg=None):
        ...
        if isinstance(ctx.channel, discord.Thread):
            for war in nation['wars']:
                if war['defender']['alliance_id'] in ['4729', '8819']:
                    if war['turnsleft'] <= 0:
        ...
        nation['offensive_wars'] = [y for y in nation['wars']
                                    if y['turnsleft'] > 0 and y['attid'] == nation['id']]
        nation['defensive_wars'] = [y for y in nation['wars']
                                    if y['turnsleft'] > 0 and y['defid'] == nation['id']]
        ...
        for war in nation['wars']:
            ...
            x['offensive_wars'] = [y for y in x['wars']
                                   if y['turnsleft'] > 0 and y['attid'] == x['id']]
            x['defensive_wars'] = [y for y in x['wars']
                                   if y['turnsleft'] > 0 and y['defid'] == x['id']]
        ...
        class status_view(discord.ui.View):
            ...
            async def general_callback(self, b: discord.Button, i: discord.Interaction):
                general_button = [
                    x for x in self.children if x.custom_id == "status_general"][0]
                military_button = [
                    x for x in self.children if x.custom_id == "status_military"][0]
    ...
    async def counters(self, ctx, *, arg):
        ...
        embed = discord.Embed(title="Counters",
                              description=f"[Explore counters against {result['nation_name']} on slotter](https://slotter.bsnk.dev/search?nation={result['id']}&alliances=4729,8819&countersMode=true&threatsMode=false&vm=false&grey=true&beige=false)", color=0x00ff00)
    ...
    async def allcounters(self, ctx, *, arg):
        ...
        embed = discord.Embed(title="Sphere Counters",
                              description=f"[Explore counters against {result['nation_name']} on slotter](https://slotter.bsnk.dev/search?nation={result['id']}&alliances=4729,8819,790,5012,2358,6877,8804&countersMode=true&threatsMode=false&vm=false&grey=true&beige=false)", color=0x00ff00)
        await ctx.send(embed=embed)
    ...
    async def member_wars(self, ctx):
        ...
        async with aiohttp.ClientSession() as session:
            query = "{wars(first: 500, alliance_id: 8819, page: page_number, active: false) {paginatorInfo {hasMorePages}data {date att_id}}}"
            n = 0
            has_more_pages = True
            wars = []
            while has_more_pages:
                ...
                async with session.post(f"https://api.politicsandwar.com/graphql?api_key={api_key}", json={'query': query.replace("page_number", str(n))}) as temp:
                    try:
                        ...
                        print((await temp.json())['errors'])
                        return
            async with session.post(f"https://api.politicsandwar.com/graphql?api_key={api_key}", json={'query': "{nations(first:500 alliance_id:8819 alliance_position:[2,3,4,5]){data{id nation_name num_cities leader_name}}}"}) as temp:
                try:
            ...
            for member in members:
                ...
                fields.append({'name': f"{member['nation_name']} ({member['id']})",
                              'value': f"{war_count} wars, c{member['num_cities']}, {discord_account}", 'wars': war_count})
    ...
    async def battlesim(self, ctx, nation1=None, nation2=None):
        ...
        embed = discord.Embed(
            title="Battle Simulator", description=f"These are the results for when [{results['nation1']['nation_name']}](https://politicsandwar.com/nation/id={results['nation1']['id']}){results['nation1_append']} attacks [{results['nation2']['nation_name']}](https://politicsandwar.com/nation/id={results['nation2']['id']}){results['nation2_append']}\nIf you want to use custom troop counts, you can use the [in-game battle simulators](https://politicsandwar.com/tools/)", color=0x00ff00)
        embed1 = discord.Embed(
            title="Battle Simulator", description=f"These are the results for when [{results['nation2']['nation_name']}](https://politicsandwar.com/nation/id={results['nation2']['id']}){results['nation2_append']} attacks [{results['nation1']['nation_name']}](https://politicsandwar.com/nation/id={results['nation1']['id']}){results['nation1_append']}\nIf you want to use custom troop counts, you can use the [in-game battle simulators](https://politicsandwar.com/tools/)", color=0x00ff00)
        ...
        class webraid(MethodView):
            def get(raidclass):
                ...
                result = Template(template).render(
                    results=results, weird_division=utils.weird_division)
                return str(result)
    ...
    async def battle_calc(self, nation1_id, nation2_id):
        ...
        for attacker, defender in [("nation1", "nation2"), ("nation2", "nation1")]:
            defender_tanks_value = results[defender]['tanks'] * \
                40 * results[f'{defender}_tanks']
            defender_soldiers_value = results[defender]['soldiers'] * \
                1.75 + results[defender]['population'] * 0.0025
            defender_army_value = defender_soldiers_value + defender_tanks_value

            attacker_tanks_value = results[attacker]['tanks'] * \
                40 * results[f'{attacker}_tanks']
            attacker_soldiers_value = results[attacker]['soldiers'] * 1.75
            attacker_army_value = attacker_soldiers_value + attacker_tanks_value

            results[f'{attacker}_ground_win_rate'] = self.winrate_calc(
                attacker_army_value, defender_army_value)
            results[f'{attacker}_ground_it'] = results[f'{attacker}_ground_win_rate']**3
            results[f'{attacker}_ground_mod'] = results[f'{attacker}_ground_win_rate']**2 * \
                (1 - results[f'{attacker}_ground_win_rate']) * 3
            results[f'{attacker}_ground_pyr'] = results[f'{attacker}_ground_win_rate'] * \
                (1 - results[f'{attacker}_ground_win_rate'])**2 * 3
            results[f'{attacker}_ground_fail'] = (
                1 - results[f'{attacker}_ground_win_rate'])**3

            results[f'{attacker}_air_win_rate'] = self.winrate_calc(
                (results[f'{attacker}']['aircraft'] * 3), (results[f'{defender}']['aircraft'] * 3))
            results[f'{attacker}_air_it'] = results[f'{attacker}_air_win_rate']**3
            results[f'{attacker}_air_mod'] = results[f'{attacker}_air_win_rate']**2 * \
                (1 - results[f'{attacker}_air_win_rate']) * 3
            results[f'{attacker}_air_pyr'] = results[f'{attacker}_air_win_rate'] * \
                (1 - results[f'{attacker}_air_win_rate'])**2 * 3
            results[f'{attacker}_air_fail'] = (
                1 - results[f'{attacker}_air_win_rate'])**3

            results[f'{attacker}_naval_win_rate'] = self.winrate_calc(
                (results[f'{attacker}']['ships'] * 4), (results[f'{defender}']['ships'] * 4))
            results[f'{attacker}_naval_it'] = results[f'{attacker}_naval_win_rate']**3
            results[f'{attacker}_naval_mod'] = results[f'{attacker}_naval_win_rate']**2 * \
                (1 - results[f'{attacker}_naval_win_rate']) * 3
            results[f'{attacker}_naval_pyr'] = results[f'{attacker}_naval_win_rate'] * \
                (1 - results[f'{attacker}_naval_win_rate'])**2 * 3
            results[f'{attacker}_naval_fail'] = (
                1 - results[f'{attacker}_naval_win_rate'])**3

            if results['gc'] == results[attacker]:
                results[f'{attacker}_ground_{defender}_avg_aircraft'] = avg_air = round(min(
                    results[f'{attacker}']['tanks'] * 0.005 * results[f'{attacker}_ground_win_rate'] ** 3, results[defender]['aircraft']))
            ...
            for type, cas_rate in [("avg", 0.7), ("diff", 0.3)]:
                results[f'{attacker}_ground_{attacker}_{type}_soldiers'] = min(round(
                    ((defender_soldiers_value * 0.0084) + (defender_tanks_value * 0.0092)) * cas_rate * 3), results[attacker]['soldiers'])
                results[f'{attacker}_ground_{attacker}_{type}_tanks'] = min(round((((defender_soldiers_value * 0.0004060606) + (defender_tanks_value * 0.00066666666)) * results[f'{attacker}_ground_win_rate'] + (
                    (defender_soldiers_value * 0.00043225806) + (defender_tanks_value * 0.00070967741)) * (1 - results[f'{attacker}_ground_win_rate'])) * cas_rate * 3), results[attacker]['tanks'])
                results[f'{attacker}_ground_{defender}_{type}_soldiers'] = min(round(
                    ((attacker_soldiers_value * 0.0084) + (attacker_tanks_value * 0.0092)) * cas_rate * 3), results[defender]['soldiers'])
                results[f'{attacker}_ground_{defender}_{type}_tanks'] = min(round((((attacker_soldiers_value * 0.00043225806) + (attacker_tanks_value * 0.00070967741)) * results[f'{attacker}_ground_win_rate'] + (
                    (attacker_soldiers_value * 0.0004060606) + (attacker_tanks_value * 0.00066666666)) * (1 - results[f'{attacker}_ground_win_rate'])) * cas_rate * 3), results[defender]['tanks'])

            results[f'{attacker}_airtoair_{attacker}_avg'] = min(round(
                results[f'{defender}']['aircraft'] * 3 * 0.7 * 0.01 * 3 * results[f'{attacker}_extra_cas']), results[f'{attacker}']['aircraft'])
            results[f'{attacker}_airtoair_{attacker}_diff'] = min(round(
                results[f'{defender}']['aircraft'] * 3 * 0.3 * 0.01 * 3 * results[f'{attacker}_extra_cas']), results[f'{attacker}']['aircraft'])
            results[f'{attacker}_airtoother_{attacker}_avg'] = min(round(
                results[f'{defender}']['aircraft'] * 3 * 0.7 * 0.015385 * 3 * results[f'{attacker}_extra_cas']), results[f'{attacker}']['aircraft'])
            results[f'{attacker}_airtoother_{attacker}_diff'] = min(round(
                results[f'{defender}']['aircraft'] * 3 * 0.3 * 0.015385 * 3 * results[f'{attacker}_extra_cas']), results[f'{attacker}']['aircraft'])

            results[f'{attacker}_airtoair_{defender}_avg'] = min(round(
                results[f'{attacker}']['aircraft'] * 3 * 0.7 * 0.018337 * 3), results[f'{defender}']['aircraft'])
            results[f'{attacker}_airtoair_{defender}_diff'] = min(round(
                results[f'{attacker}']['aircraft'] * 3 * 0.3 * 0.018337 * 3), results[f'{defender}']['aircraft'])
            results[f'{attacker}_airtoother_{defender}_avg'] = min(round(
                results[f'{attacker}']['aircraft'] * 3 * 0.7 * 0.009091 * 3), results[f'{defender}']['aircraft'])
            results[f'{attacker}_airtoother_{defender}_diff'] = min(round(
                results[f'{attacker}']['aircraft'] * 3 * 0.3 * 0.009091 * 3), results[f'{defender}']['aircraft'])

            results[f'{attacker}_naval_{defender}_avg'] = min(round(
                results[f'{attacker}']['ships'] * 4 * 0.7 * 0.01375 * 3 * results[f'{attacker}_extra_cas']), results[f'{defender}']['aircraft'])
            results[f'{attacker}_naval_{defender}_diff'] = min(round(
                results[f'{attacker}']['ships'] * 4 * 0.3 * 0.01375 * 3 * results[f'{attacker}_extra_cas']), results[f'{defender}']['aircraft'])
            results[f'{attacker}_naval_{attacker}_avg'] = min(round(
                results[f'{defender}']['ships'] * 4 * 0.7 * 0.01375 * 3), results[f'{attacker}']['aircraft'])
            results[f'{attacker}_naval_{attacker}_diff'] = min(round(
                results[f'{defender}']['ships'] * 4 * 0.3 * 0.01375 * 3), results[f'{attacker}']['aircraft'])
        ...
        results["nation1"]['city'] = sorted(
            results['nation1']['cities'], key=lambda k: k['infrastructure'], reverse=True)[0]
        results["nation2"]['city'] = sorted(
            results['nation2']['cities'], key=lambda k: k['infrastructure'], reverse=True)[0]
        ...
        for attacker, defender in [("nation1", "nation2"), ("nation2", "nation1")]:
            results[f'{attacker}_ground_{defender}_lost_infra_avg'] = max(min(((results[f'{attacker}']['soldiers'] - results[f'{defender}']['soldiers'] * 0.5) * 0.000606061 + (results[f'{attacker}']['tanks'] - (results[f'{defender}']['tanks'] * 0.5)) * 0.01)
                                                                          * 0.95 * results[f'{attacker}_ground_win_rate'], results[defender]['city']['infrastructure'] * 0.2 + 25), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost']
            results[f'{attacker}_ground_{defender}_lost_infra_diff'] = results[f'{attacker}_ground_{defender}_lost_infra_avg'] / 0.95 * 0.15
            results[f'{attacker}_ground_loot_avg'] = (results[f'{attacker}']['soldiers'] * 1.1 + results[f'{attacker}']['tanks'] * 25.15) * (
                results[f'{attacker}_ground_win_rate'] ** 3) * 3 * 0.95 * results[f'{attacker}_war_loot_mod'] * results[f'{attacker}_policy_loot_stolen'] * results[f'{defender}_policy_loot_lost']
            results[f'{attacker}_ground_loot_diff'] = results[f'{attacker}_ground_loot_avg'] / 0.95 * 0.1

            results[f'{attacker}_air_{defender}_lost_infra_avg'] = max(min((results[f'{attacker}']['aircraft'] - results[f'{defender}']['aircraft'] * 0.5) * 0.35353535 * 0.95 * results[f'{attacker}_air_win_rate'],
                                                                       results[defender]['city']['infrastructure'] * 0.5 + 100), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost']
            results[f'{attacker}_air_{defender}_lost_infra_diff'] = results[f'{attacker}_air_{defender}_lost_infra_avg'] / 0.95 * 0.15
            results[f'{attacker}_air_{defender}_soldiers_destroyed_avg'] = round(max(min(results[f'{defender}']['soldiers'], results[f'{defender}']['soldiers'] * 0.75 + 1000, (
                results[f'{attacker}']['aircraft'] - results[f'{defender}']['aircraft'] * 0.5) * 35 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
            results[f'{attacker}_air_{defender}_soldiers_destroyed_diff'] = results[
                f'{attacker}_air_{defender}_soldiers_destroyed_avg'] / 0.95 * 0.1
            results[f'{attacker}_air_{defender}_tanks_destroyed_avg'] = round(max(min(results[f'{defender}']['tanks'], results[f'{defender}']['tanks'] * 0.75 + 10, (
                results[f'{attacker}']['aircraft'] - results[f'{defender}']['aircraft'] * 0.5) * 1.25 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
            results[f'{attacker}_air_{defender}_tanks_destroyed_diff'] = results[f'{attacker}_air_{defender}_tanks_destroyed_avg'] / 0.95 * 0.1
            results[f'{attacker}_air_{defender}_ships_destroyed_avg'] = round(max(min(results[f'{defender}']['ships'], results[f'{defender}']['ships'] * 0.75 + 4, (
                results[f'{attacker}']['aircraft'] - results[f'{defender}']['aircraft'] * 0.5) * 0.0285 * 0.95), 0)) * airstrike_casualties(results[f'{attacker}_air_win_rate'])
            results[f'{attacker}_air_{defender}_ships_destroyed_diff'] = results[f'{attacker}_air_{defender}_ships_destroyed_avg'] / 0.95 * 0.1

            results[f'{attacker}_naval_{defender}_lost_infra_avg'] = max(min((results[f'{attacker}']['ships'] - results[f'{attacker}']['ships'] * 0.5) * 2.625 * 0.95 * results[f'{attacker}_naval_win_rate'],
                                                                         results[defender]['city']['infrastructure'] * 0.5 + 25), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost']
            results[f'{attacker}_naval_{defender}_lost_infra_diff'] = results[f'{attacker}_naval_{defender}_lost_infra_avg'] / 0.95 * 0.15

            results[f'{attacker}_nuke_{defender}_lost_infra_avg'] = max(min((1700 + max(2000, results[defender]['city']['infrastructure'] * 100 / results[defender]['city']['land'] * 13.5)) / 2, results[defender]
                                                                        ['city']['infrastructure'] * 0.8 + 150), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost'] * results[f'{attacker}_vds_mod']
            results[f'{attacker}_missile_{defender}_lost_infra_avg'] = max(min((300 + max(350, results[defender]['city']['infrastructure'] * 100 / results[defender]['city']['land'] * 3)) / 2, results[defender]['city']
                                                                           ['infrastructure'] * 0.3 + 100), 0) * results[f'{attacker}_war_infra_mod'] * results[f'{attacker}_policy_infra_dealt'] * results[f'{defender}_policy_infra_lost'] * results[f'{attacker}_irond_mod']
            ...
            for infra in [
                ...
                results[f'{infra}_avg_value'] = utils.infra_cost(
                    results[defender]['city']['infrastructure'] - results[f'{infra}_avg'], results[defender]['city']['infrastructure'])
                try:
                    results[f'{infra}_diff_value'] = utils.infra_cost(
                        results[defender]['city']['infrastructure'] - results[f'{infra}_diff'], results[defender]['city']['infrastructure'])
                except:
                    pass
            ...
            results[f'{attacker}_ground_{attacker}_mun'] = results[f'{attacker}']['soldiers'] * \
                0.0002 + results[f'{attacker}']['tanks'] * 0.01
            ...
            base_mun = (results[f'{defender}']['soldiers'] * 0.0002 + results[f'{defender}']['population'] / 2000000 +
                        results[f'{defender}']['tanks'] * 0.01) * def_rss_consumption(results[f'{attacker}_ground_win_rate'])
            results[f'{attacker}_ground_{defender}_mun'] = (base_mun * (1 - results[f'{attacker}_ground_fail']) + min(
                base_mun, results[f'{attacker}_ground_{attacker}_mun']) * results[f'{attacker}_ground_fail'])
            base_gas = results[f'{defender}']['tanks'] * 0.01 * \
                def_rss_consumption(results[f'{attacker}_ground_win_rate'])
            results[f'{attacker}_ground_{defender}_gas'] = (base_gas * (1 - results[f'{attacker}_ground_fail']) + min(
                base_gas, results[f'{attacker}_ground_{attacker}_gas']) * results[f'{attacker}_ground_fail'])
            ...
            for attack in ['air', 'airvair', 'airvinfra', 'airvsoldiers', 'airvtanks', 'airvships']:
                results[f'{attacker}_{attack}_{attacker}_gas'] = results[
                    f'{attacker}_{attack}_{attacker}_mun'] = results[f'{attacker}']['aircraft'] / 4
                base_gas = results[f'{defender}']['aircraft'] / 4 * \
                    def_rss_consumption(results[f'{attacker}_air_win_rate'])
                results[f'{attacker}_{attack}_{defender}_gas'] = results[f'{attacker}_{attack}_{defender}_mun'] = (
                    base_gas * (1 - results[f'{attacker}_air_fail']) + min(base_gas, results[f'{attacker}_air_{attacker}_gas']) * results[f'{attacker}_air_fail'])
            ...
            base_mun = results[f'{defender}']['ships'] * 3 * \
                def_rss_consumption(results[f'{attacker}_air_win_rate'])
            results[f'{attacker}_naval_{defender}_mun'] = results[f'{attacker}_naval_{defender}_mun'] = (
                base_mun * (1 - results[f'{attacker}_naval_fail']) + min(base_gas, results[f'{attacker}_naval_{attacker}_mun']) * results[f'{attacker}_naval_fail'])
            base_gas = results[f'{defender}']['ships'] * 2 * \
                def_rss_consumption(results[f'{attacker}_air_win_rate'])
            results[f'{attacker}_naval_{defender}_gas'] = results[f'{attacker}_naval_{defender}_gas'] = (
                base_gas * (1 - results[f'{attacker}_naval_fail']) + min(base_gas, results[f'{attacker}_naval_{attacker}_gas']) * results[f'{attacker}_naval_fail'])
            ...
            results[f'{attacker}_naval_{defender}_money'] = results[f'{attacker}_naval_{defender}_lost_infra_avg_value'] + \
                results[f'{attacker}_naval_{defender}_avg'] * 50000
